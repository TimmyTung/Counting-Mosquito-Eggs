...
#Generates a random color for the bounding boxes and text color
def random_color():
    rgbl=[255,0,0]
    random.shuffle(rgbl)
    return tuple(rgbl)
        ...
        for i in contours:
            ...
            if(area > EGG_AREA+(EGG_AREA*.4)):
                ...
                if displayBoundingBoxes == True:
                    ...
                    cv2.putText(img,str(num),(math.floor(x+w/2)-4*thickness, math.floor(y+h/2)+4*thickness),font, 2, color, thickness)
